...
**Tools:** In the config, enable web search, browser, and memory tools. Here's what the generated `config.toml` looks like under the hood:
...
The agent gets `web_search` for finding information, `web_fetch` for reading full pages, and Playwright for navigating JavaScript-heavy sites. Memory is set to `markdown` — human-readable logs stored in `MEMORY.md` that persist across sessions.
...
**Add MCP tools.** Want your agent to query a database or call an internal API? Create an MCP server in the dashboard, link it to the agent config, and redeploy. The agent picks up the new tools automatically.
...
**Schedule research.** Use the built-in cron tools to have the agent run research tasks on a schedule and store findings in its workspace for you to review later.

## What's Next

- [Build a Discord support bot with docs search](/blog/build-discord-support-bot) — the next Build This tutorial
